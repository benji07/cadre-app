...
4. `resize` aux dimensions exactes calculées (`fit: 'fill'`), noyau lanczos3 ; en « remplir », le recadrage centré est fait avant par `extract`.
...
7. Encodage JPEG qualité 100, sous-échantillonnage 4:4:4, mozjpeg. Un seul encodage : la qualité est maximale et le poids du fichier n'est pas plafonné.
...
## Valeurs par défaut
Dossier `~/Pictures/Instagram`, taille 1×, EXIF supprimées, JPEG 100.
...
- Le poids du fichier n'est pas contraint : en 2×, un export peut dépasser la limite de 8 Mo d'Instagram (voir `features/formats.md`), qui recompressera alors le fichier.
